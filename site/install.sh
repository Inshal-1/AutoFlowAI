...
REPO="https://github.com/p0tfur/AutoFlow.git"
INSTALL_DIR="autoflow"
MIN_BUN_MAJOR=1
...
printf "\n${BOLD}autoflow${RESET} ${DIM}— ai agent for android${RESET}\n\n"
...
# ─── check/install bun ───
# autoflow requires bun — it uses Bun.spawnSync() and native .env loading
...
if command -v bun >/dev/null 2>&1; then
  ...
  if command -v node >/dev/null 2>&1; then
    warn "node found but autoflow requires bun (uses bun-specific APIs)"
...
printf "next steps:\n\n"
printf "  ${BOLD}1.${RESET} configure an llm provider in ${CYAN}autoflow/.env${RESET}\n\n"
...
printf "     cd autoflow && bun run src/kernel.ts\n\n"
printf "docs: ${CYAN}https://pa.rpaby.pw${RESET}\n"
printf "repo: ${CYAN}https://github.com/p0tfur/AutoFlow${RESET}\n\n"
